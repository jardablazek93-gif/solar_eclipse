...
st.set_page_config(
    page_title="Solar Eclipse Explorer",
    page_icon="☀",
    layout="wide",
...
# A small amount of CSS makes the default Streamlit layout feel more like a
# finished application without fighting Streamlit's responsive behaviour.
st.markdown(
    """
    <style>
    .block-container { max-width: 1180px; padding-top: 2.4rem; padding-bottom: 4rem; }
    [data-testid="stSidebar"] { border-right: 1px solid rgba(120,120,120,.14); }
    [data-testid="stSidebar"] .block-container { padding-top: 2rem; }
    .app-kicker { color: #d97706; font-size: .78rem; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; margin-bottom: .35rem; }
    .app-title { font-size: clamp(2rem, 4vw, 3.1rem); line-height: 1.05; font-weight: 750; letter-spacing: -.035em; margin: 0; }
    .app-lead { color: #667085; font-size: 1.03rem; max-width: 720px; margin-top: .7rem; margin-bottom: 1.8rem; line-height: 1.6; }
    .section-note { color: #667085; font-size: .9rem; margin-top: -.5rem; margin-bottom: 1rem; }
    .result-card { border: 1px solid rgba(120,120,120,.16); border-radius: 14px; padding: 1rem 1.1rem; background: rgba(255,255,255,.02); }
    .result-card-label { color: #667085; font-size: .78rem; text-transform: uppercase; letter-spacing: .07em; }
    .result-card-value { font-size: 1.55rem; font-weight: 700; margin-top: .15rem; }
    .muted { color: #667085; }
    div[data-testid="stMetric"] { border: 1px solid rgba(120,120,120,.14); padding: .8rem 1rem; border-radius: 12px; }
...
TEXTS = {
    "CZ": {
        "page_title": "Solar Eclipse Explorer",
        "page_desc": "Najděte zatmění Slunce pozorovatelná z libovolného místa na Zemi a prohlédněte si jejich průběh v čase.",
        "sidebar_header": "Nastavení výpočtu",
        ...
        "sidebar_lat": "Šířka",
        "sidebar_lon": "Délka",
        "sidebar_elev": "Nadmořská výška",
        "location_ready": "Poloha je připravena",
        "no_eclipses": "Pro zadanou polohu a časový rozsah nebylo nalezeno žádné viditelné zatmění nad obzorem.",
        "found_eclipses": "Viditelná zatmění",
        "found_note": "Výsledky jsou počítány pro zadanou polohu a časové období.",
        "col_date": "Datum",
        "col_time": "Maximum (UTC)",
        "col_type": "Typ",
        "col_obs": "Zakrytí",
        "col_alt": "Výška Slunce",
        "sim_header": "Průběh vybraného zatmění",
        "sim_note": "Posuňte čas a sledujte, jak se mění poloha Měsíce vůči Slunci.",
        "select_eclipse": "Zatmění",
        "slider_label": "Čas (UTC)",
        "chart_title": "Zakrytí Slunce a jeho výška nad obzorem",
        "chart_xlabel": "Čas (UTC)",
        "chart_ylabel_obs": "Zakrytí Slunce (%)",
        "chart_ylabel_alt": "Výška Slunce (°)",
        "sim_title": "Pohled na Slunce",
        ...
        "summary_count": "Počet zatmění",
        "summary_best": "Nejvyšší zakrytí",
        "summary_first": "První zatmění",
        "summary_last": "Poslední zatmění",
        "summary_location": "Poloha",
    },
    "EN": {
        "page_title": "Solar Eclipse Explorer",
        "page_desc": "Find solar eclipses visible from any location on Earth and explore how each eclipse develops over time.",
        "sidebar_header": "Calculation settings",
        ...
        "location_ready": "Location is ready",
        "no_eclipses": "No visible solar eclipses were found above the horizon for the selected location and time range.",
        "found_eclipses": "Visible eclipses",
        "found_note": "Results are calculated for the selected location and time period.",
        "col_date": "Date",
        "col_time": "Peak (UTC)",
        "col_type": "Type",
        "col_obs": "Obscuration",
        "col_alt": "Sun altitude",
        "sim_header": "Selected eclipse",
        "sim_note": "Move through time to see the Moon's position relative to the Sun.",
        "select_eclipse": "Eclipse",
        "slider_label": "Time (UTC)",
        "chart_title": "Solar obscuration and altitude",
        "chart_xlabel": "Time (UTC)",
        "chart_ylabel_obs": "Obscuration (%)",
        "chart_ylabel_alt": "Sun altitude (°)",
        "sim_title": "View of the Sun",
        ...
        "summary_count": "Eclipses",
        "summary_best": "Highest obscuration",
        "summary_first": "First eclipse",
        "summary_last": "Last eclipse",
...
def parse_coordinates(coord_str):
    ...
    def parse_part(part, pos_dirs, neg_dirs):
        ...
        if direction in neg_dirs:
            val = -abs(val)
        elif direction in pos_dirs:
            val = abs(val)
        return val
        ...
        for idx in range(len(t_search)):
            ...
            if d_val < (s_rad + m_rad):
                ...
                if obscuration > max_obscuration:
                    ...
                    if obscuration >= 99.9:
                        type_key = "type_total" if m_rad >= s_rad else "type_annular"
...
def compute_eclipse_frames(lat, lon, elevation, peak_dt):
    ...
    t_range = ts.utc(
        ...
        start_dt.day,
        start_dt.hour,
        start_dt.minute + np.array(minutes),
    )
...
# -----------------------------------------------------------------------------
with st.sidebar:
    st.markdown("### Solar Eclipse Explorer")
    st.caption("Astronomical calculator and simulation")

    lang = st.radio(TEXTS["CZ"]["lang_label"], ["CZ", "EN"], horizontal=True)
    t = TEXTS[lang]
    ...
    coord_input = st.text_input(
        t["coord_label"],
        value="50.0835494N, 14.4341414E",
        help="Např. 50.0835494N, 14.4341414E",
    )
...
# Main header
# -----------------------------------------------------------------------------
st.markdown("<div class='app-kicker'>Astronomical calculator</div>", unsafe_allow_html=True)
st.markdown(f"<h1 class='app-title'>{t['page_title']}</h1>", unsafe_allow_html=True)
st.markdown(f"<div class='app-lead'>{t['page_desc']}</div>", unsafe_allow_html=True)
...
try:
    ...
    st.sidebar.success(
        f"{t['location_ready']}\n\n"
        f"{t['sidebar_lat']}: {lat:.4f}°\n\n"
        f"{t['sidebar_lon']}: {lon:.4f}°\n\n"
        f"{t['sidebar_elev']}: {elevation:.1f} m"
    )
...
if btn_compute or "eclipse_results" in st.session_state:
    ...
    if not results:
        ...
        # Keep the result overview compact and easy to scan.
        best = max(results, key=lambda r: r["obscuration_val"])
        c1, c2, c3, c4 = st.columns(4)
        c1.metric(t["summary_count"], len(results))
        c2.metric(t["summary_best"], f"{best['obscuration_val']:.1f} %")
        c3.metric(t["summary_first"], results[0]["date"])
        c4.metric(t["summary_last"], results[-1]["date"])

        st.markdown("### " + t["found_eclipses"])
        st.markdown(f"<div class='section-note'>{t['found_note']}</div>", unsafe_allow_html=True)
        ...
        st.divider()
        st.markdown("### " + t["sim_header"])
        st.markdown(f"<div class='section-note'>{t['sim_note']}</div>", unsafe_allow_html=True)

        options = [
            f"{r['date']}  ·  {t.get(r['type_key'], '')}  ·  {r['obscuration']}"
            for r in results
        ]
        ...
        # A small live summary above the plots gives the user the important
        # information without forcing them to read the chart.
        s1, s2, s3 = st.columns(3)
        s1.metric(t["sim_time"], current_frame["time_str"])
        s2.metric(t["sim_obs"], f"{current_frame['obscuration']:.2f} %")
        s3.metric(t["sim_alt"], f"{current_frame['alt']:.1f}°")

        col_g1, col_g2 = st.columns([1.15, 1], gap="large")

        with col_g1:
            fig_graph, ax_graph = plt.subplots(figsize=(6.4, 4.3))
            obscuration_vals = [f["obscuration"] for f in frames_data]

            ax_graph.set_xlabel(t["chart_xlabel"], fontsize=9)
            ax_graph.set_ylabel(t["chart_ylabel_obs"], fontsize=9)
            ax_graph.plot(dt_list, obscuration_vals, linewidth=2.2)
            ax_graph.fill_between(dt_list, obscuration_vals, alpha=0.12)
            ax_graph.set_ylim(-2, 105)
            ax_graph.grid(axis="y", alpha=0.18)

            ax_graph_alt = ax_graph.twinx()
            ax_graph_alt.set_ylabel(t["chart_ylabel_alt"], fontsize=9)
            ax_graph_alt.plot(dt_list, altitudes, linestyle="--", linewidth=1.2, alpha=0.75)
            ax_graph_alt.axhline(0, linestyle=":", linewidth=1, alpha=0.55)
            ax_graph_alt.grid(False)

            ax_graph.axvline(current_frame["dt"], linestyle="-", linewidth=1.8, alpha=0.8)
            ax_graph.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
            ax_graph.set_title(t["chart_title"], fontsize=10, fontweight="bold", pad=12)
            fig_graph.tight_layout()
            st.pyplot(fig_graph, clear_figure=True)

        with col_g2:
            fig_sim, ax_sim = plt.subplots(figsize=(5.5, 4.3))
            ax_sim.set_facecolor("#101318")
            ...
            moon_patch = Circle(
                (current_frame["delta_ra"], current_frame["delta_dec"]),
                current_frame["moon_r"],
                color="#171a20",
                ec="#9aa3b2",
                lw=1.1,
                zorder=2,
            ...
            sim_title_text = (
                f"{t['sim_title']}\n"
                f"{current_frame['time_str']}  ·  {current_frame['obscuration']:.2f}%"
            )
            ax_sim.set_title(sim_title_text, fontsize=9, fontweight="bold", color="white", pad=12)
